/**
 * Modal de edicion de pedidos existentes.
 *
 * Clon visual de `ManualOrderModal.jsx`. Diferencias:
 * - Usa `useOrderEdit` (precarga desde `order` prop) en vez de `useManualOrder`.
 * - Acepta prop `order` con el pedido a editar.
 * - Boton final dice "GUARDAR CAMBIOS" en vez de "CONFIRMAR PEDIDO".
 * - No registra venta en caja (esa transicion la hace `moveOrder` -> active).
 *
 * Reusa los mismos estilos `manual-order-*` y el mismo grid de productos
 * para que el cajero no note diferencias entre crear y editar.
 */
...
const OrderEditModal = ({ isOpen, onClose, order, products, categories = [], onOrderSaved, showNotify, branch, logoUrl, companyName, moveOrder }) => {
	...
	} = useOrderEdit(showNotify, onOrderSaved, onClose, branch, branchDeliveryCfg, order);

	const [showCustomerFields, setShowCustomerFields] = useState(false);
	const [detectingZone, setDetectingZone] = useState(false);
	const [calculatingDistance, setCalculatingDistance] = useState(false);
	// Comentarios por item: estado de toggle. Si el item ya trae nota, abierto.
	...
	// Validacion de EDICION (mas laxa que la de creacion manual):
	// - Obligatorios: items, nombre, payment_type, delivery valido si aplica.
	// - Opcionales: RUT y telefono. Si el cajero los llena se validan; si los
	//   deja vacios se permite guardar (hay pedidos web sin esos campos).
	const isFormValid = () => {
		...
		return hasItems && hasClientName && hasPaymentType && rutOk && phoneOk && isDeliveryValid;
	};

	// Cancelar el pedido desde el modal. Reusamos `moveOrder(id, 'cancelled')`
	// del provider, que ya dispara `cashSystem.registerRefund` si el pedido ya
	// habia registrado venta (active/completed/picked_up).
	...
	const handleCancelOrder = async () => {
		if (!canCancelOrder || cancelling || loading) return;
		...
		const stageLabel =
			...
			status;
		const refundWarning = ['active', 'completed', 'picked_up'].includes(status)
			? '\n\nEste pedido ya estaba en caja: se registrara una DEVOLUCION automatica.'
			: '';
		const ok = typeof window !== 'undefined'
			? window.confirm(
				`Cancelar pedido #${order.id} (estado: ${stageLabel})?` + refundWarning,
			)
			: true;
		if (!ok) return;
		setCancelling(true);
		try {
			await moveOrder(order.id, 'cancelled');
			if (onClose) onClose();
		} finally {
			setCancelling(false);
		}
	...
	const describeValidationIssue = () => {
		if (!manualOrder.items || manualOrder.items.length === 0) return 'Agrega al menos un producto.';
		if (!manualOrder.client_name || manualOrder.client_name.trim().length < 3) return 'Falta el nombre del cliente (min. 3 caracteres).';
		if (!manualOrder.payment_type) return 'Selecciona metodo de pago.';
		const rutRaw = String(manualOrder.client_rut ?? '').trim();
		if (rutRaw && rutValid !== true) return 'El RUT no es valido. Borralo o corrigelo.';
		const phoneRaw = String(manualOrder.client_phone ?? '').trim();
		const phoneDigits = phoneRaw.replace(/\D/g, '').length;
		if (phoneRaw && phoneDigits > 0 && phoneValid !== true) return 'Telefono incompleto. Borralo o completalo a +56 9 XXXX XXXX.';
		if (manualOrder.order_type === 'delivery') {
			...
			const zoneId = String(manualOrder.delivery_named_area_id ?? '').trim();
			if (namedMode && !zoneId) return 'Selecciona una zona de entrega.';
			if (distMode && addr.length < 5) return 'Falta direccion de entrega (min. 5 caracteres).';
			if (!namedMode && !distMode && branchDeliveryCfg && !zoneId && addr.length < 5) {
				return 'Falta direccion o zona de entrega.';
			}
			if (!branchDeliveryCfg && addr.length < 5) return 'Falta direccion de entrega.';
		}
	...
	const footerSection = (
		<div className="manual-order-footer">
			...
			{/* En EDICION dejamos el boton siempre activo (excepto loading) y
			    delegamos la validacion a `submitOrder`, que muestra un toast
			    concreto explicando que falta. Asi el cajero ve por que no
			    se guardo en vez de un boton mudo. */}
			<button
			...
			{canCancelOrder ? (
				<button
					...
					style={{
						...
						opacity: cancelling || loading ? 0.6 : 1,
	...
	// Portal a document.body: el card del kanban tiene `animate-slide-up` con
	// `transform` que crea un containing block para descendientes `position: fixed`,
	// asi que el overlay del modal quedaba atrapado dentro de la card en vez de
	// cubrir la pantalla. Ademas, replicamos las clases `tenant-theme-vars` y
	// `admin-layout` para que las variables CSS de tema (tenant + tema claro del
	// admin) sigan aplicando dentro del portal.
	return createPortal(
		<div className="tenant-theme-vars admin-layout">
							...
							{!hasAnyResults ? (
								...
								<>
									<div ref={productsSectionRef}>
										{renderCatalogSection(groupedBaseCatalog, 'Productos', '', 'products', 'Producto regular del menú para este pedido.')}
									</div>
